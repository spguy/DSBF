...
[fval_base] = score(x_base,sat_num,x_ground,y_ground,node_x,node_y,sattochoose_x,sattochoose_y,power_base,frequency,D);
disp(['无优化分数：', num2str(-fval_base)])%pi


phii = x_base(1:sat_num);
satidxi = x_base(sat_num+1:sat_num*2);  
%避免重复
for i = 1:sat_num-1
    if(satidxi(i+1)==satidxi(i))
        satidxi(i+1) = satidxi(i+1)+1;
    end
end
sat_loc_xi = sattochoose_x(round(satidxi));
sat_loc_yi = sattochoose_y(round(satidxi));
...
options = optimoptions('ga','PopulationSize',80,'MaxTime',0.1);%default: ps 200; mg 200*number of variable %'MaxGenerations',100,
%options = optimoptions('ga');
%tic;
tstart = cputime;
[x, fval,exitflag,output,population,scores] = ga(targetFunction, nvars, [], [], [], [], LB, UB,[],options);
%calculation_time = toc%记录计算时间
cpu_usage_ga = cputime - tstart
[fval,power,propability] = score(x,sat_num,x_ground,y_ground,node_x,node_y,sattochoose_x,sattochoose_y,power_base,frequency,D);%算出最优情况下的各项指标
fval
...
%粒子群对比组
system('wmic cpu get loadpercentage')
options = optimoptions('particleswarm','SwarmSize',80,'MaxTime',0.1);%Default is min(100,10*nvars) %,'MaxIterations',100
tic;
tstart = cputime;
...
%模拟退火对比组
system('wmic cpu get loadpercentage')
options = optimoptions('simulannealbnd','MaxTime',0.1);
tic;
tstart = cputime;
[x, fval,exitflag,output] = simulannealbnd(targetFunction, LB, LB, UB ,options);
calculation_time = toc%记录计算时间
cpu_usage_ga = cputime - tstart
[fval,power,propability] = score(x,sat_num,x_ground,y_ground,node_x,node_y,sattochoose_x,sattochoose_y,power_base,frequency,D);%算出最优情况下的各项指标
fval
...
optimalSatTrack = sattrack_theta(optimalidx);
...
phi = xx(1:sat_num);
satidx = round(xx(sat_num+1:sat_num*2));  
sat_loc_x = x_sat(round(satidx));
sat_loc_y = y_sat(round(satidx));
